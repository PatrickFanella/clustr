...
func Load() *Config {
	...
	cached = &Config{
		...
		GraphQueryTimeout:     time.Duration(utils.GetEnvAsInt("GRAPH_QUERY_TIMEOUT_MS", 5000)) * time.Millisecond,
		DBStatementTimeout:    time.Duration(utils.GetEnvAsInt("DB_STATEMENT_TIMEOUT_MS", 5000)) * time.Millisecond,
